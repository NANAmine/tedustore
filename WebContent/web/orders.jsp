			...
			<c:forEach items="${orders}" var="order" varStatus="orderStatus">
				<div id="orderItem">
					<p class="orderItem_title">
						<span id="order_id"> &nbsp;&nbsp;订单编号:<a href="#">${order.id   }</a>
						</span> &nbsp;&nbsp;成交时间：${order.tradeTime}&nbsp;&nbsp; <span> <a
				...
				<c:forEach items="${allOrderItems[orderStatus.index]}" var="orderItem"
					varStatus="orderItemStatus">
					<div id="orderItem_detail">
						<ul>
							<li class="product"><b><a href="#">
							 <img
										src="..${orderItem.image}" width="84px" height="84px" /></a></b> <b
								class="product_name lf"> <a href="">${orderItem.title }</a>
									<br />
							</b> <b class="product_color "> 规格：${orderItem.spec} </b></li>
							<li class="unit_price">专属价 <br /> ￥${orderItem.price }
							</li>
							<li class="count">${orderItem.count }件</li>
							<li class="sale_support">退款/退货 <br /> 我要维权
							</li>

							<li class=" payments_received">
								￥${orderItem.count*orderItem.price }</li>
							<c:if test="${orderItemStatus.index == 0 }">
								<li class="trading_status"><a
									href="showOrderInfo.do?orderid=${orderItem.orderId }">订单详情</a>
									<br /> <c:choose>
										...
										<c:when test="${order.shippingStatus==2}">
				                          ...
				                          <br />
											<a class="view_logistics"
												href="showOrderInfo.do?orderid=${orderItem.orderId }">查看物流</a>
										</c:when>
										<c:when test="${order.shippingStatus==3}">
		                                  已交付
		                              </c:when>
										<c:when test="${order.shippingStatus==4}">
		                                  已退回
		                              </c:when>
										<c:when test="${order.shippingStatus==5}">
		                                  已取消
		                              </c:when>
									</c:choose></li>
								<li class="operate"><c:if
										test="${order.shippingStatus==0 || order.shippingStatus==1}">
										<a href="cancelOrder.do?orderId=${orderItem.orderId}">取消订单</a>
									</c:if> <c:if test="${order.shippingStatus==2}">
										<a href="confirmReceived.do?orderId=${orderItem.orderId}">确认收货</a>
									</c:if> <c:if test="${order.shippingStatus==3}">
										<a href="#">评价</a>
									</c:if></li>
							</c:if>
						</ul>
