...
<!--细节导航-->
<div id="nav_detail">
    <h5>首页 > 学习用品 > ${goods.itemType } > ${goods.title }></h5>
</div>
<!--产品预览-->
<div id="shop_detail">
    <!-- 左侧-->
    <div id="preview" class="lf">
        ...
        <div id="icon_all">
            <ul id="icon_list">
            	<c:forEach items="${goodsPictures }" var="pic" varStatus="s">
	                <li class="i1"><img width="84px" src="${pageContext.request.contextPath}/${pic.url}"/></li>
            	</c:forEach>
            </ul>
        </div>
    </div>
    ...
    <div class="right_detail lf">
        ...
        <p class="spec">
            ...
            <c:forEach items="${goodsSpecVO}" var="spec">
            	<c:if test="${spec.id==goods.id}">
					<a href="showGoodsInfo.do?goodsId=${spec.id}&categoryId=${goods.categoryId}" 
							class="avenge borderChange" style="border:width:auto; height:auto; 1px solid rgb(102, 102, 102); color: rgb(102, 102, 102);">
						${spec.spec}
					</a>
            	</c:if>
            	<c:if test="${spec.id != goods.id}">
					<a href="showGoodsInfo.do?goodsId=${spec.id}&categoryId=${goods.categoryId}" 
							class="avenge" style="border:width:auto;height:auto; 1px solid rgb(102, 102, 102); color: rgb(102, 102, 102);">
						${spec.spec}
					</a>
            	</c:if>
            </c:forEach>
        ...
        <p class="accountChose">
         ...
         <!-- 购买部分-->
        <div class="shops" style="margin-top: 10px !important">
...
<div id="recommended">
    ...
    <div id="demo" style="width:1000px;overflow:hidden;">
        <div id="indemo" style="float: left;width: 200%;">
            ...
            <!-- 第一个宽度显示 -->
                 <c:forEach items="${recommendGoodsVO}" var="oneGoods">
	                 <div class="detail_1 lf">
	                    <div class="detail_img1">
	                    	<a href="showGoodsInfo.do?goodsId=${oneGoods.id}&categoryId=${oneGoods.categoryId}">
	                        	<img src="..${oneGoods.image}" border="0">
	                        </a>
	                    </div>
	                    <p style="margin-top:auto">${oneGoods.title }</p>
	                </div>
                </c:forEach>
                <!--</div>-->
            </div>
